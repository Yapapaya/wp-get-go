...
// initialise module to fetch starter package
var componentFetcher = require( './component-fetcher' );
...
wpGG.fetch = function() {
	...
	var fetchStarter = starterThemeFetcher.fetch( bootStrapInfo, options, function() {
		starterThemeReplacer.replaceTheme( packageInfo, options );
	} );

	var fetchComponents = componentFetcher.fetch( packageInfo, options, function( downloaded, componentErrors ) {
		
		bootStrapInfo.downloaded = downloaded;

		Object.keys( componentErrors ).forEach( function( key ) {
			
			log.warn(componentErrors[key]);
	...
	Q.all([ fetchStarter(), fetchComponents() ])
		//.then(function(){
		//buildTheme()
		//})
.done();
