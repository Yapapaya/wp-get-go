 ...
 * @returns {object}
 */
wpggBuildInfoParser.parse = function( json ) {
	...
	if ( this.options.push === false ) {
		...
		if ( this.packageInfo.shouldPush === false ) {
			...
			log.info( "A repository wasn't specified or had an error. Can\'t push; please commit and push manually." );
		} else {
...
wpggBuildInfoParser.validateName = function() {
	...
	if ( ( invalidCharsFound !== -1 ) ) {
		...
		if ( this.options.autofix === false ) {
			...
			log.info('Package name will be updated from '+ ogPackageName + ' to ' + this.packageInfo.name );
		}
	}
