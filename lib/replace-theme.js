...
wpbsReplacer.defReplaceOptions = {
	...
	recursive: true,
	silent: false,
	exclude: "./Gruntfile.js, ./package.json, ./.gitignore, node_modules/*, .svn"
};
...
wpbsReplacer.replaceStylesheet = function() {
	...
	Object.keys( themeHeaders ).forEach( function( themeHeader ) {
		...
		grunt.verbose.writeln( 'Replacing ' + chalk.yellow( replaceOptions.regex ) + ' with ' + chalk.yellow( replaceOptions.replacement ) );
...
wpbsReplacer.replaceName = function( name ) {
	...
	Object.keys( regexReplace ).forEach( function( key ) {
		...
		grunt.verbose.writeln( 'Replacing ' + chalk.yellow( key ) + ' with ' + chalk.yellow( regexReplace[key] ) );
